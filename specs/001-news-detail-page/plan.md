...
## Summary
Implement a dedicated detail page for news articles using `react-router-dom` for navigation and a decoupled service layer to fetch full article content. This plan includes updating the existing mock data and components to support linking and deep-linking.
...
- **II. Decoupled Service**: New method `getNewsById` will be added to `newsService.ts`.
...
│       └── NewsPage.styles.css
├── services/
│   └── newsService.ts      # [MODIFY] Added getNewsById logic
└── App.tsx                 # [MODIFY] Added Routes structure
```
...
- Updated `NewsDTO` to support large content blocks.
- Defined the `/news/:id` route contract.

### Phase 2: Execution
1. **Infrastructure**: Install `react-router-dom` and configure `BrowserRouter` in `main.tsx`.
2. **Data Update**: Update `mockData.json` with multi-line `fullContent` for all items.
...
- Direct URL entry for non-existent IDs (verify 404/Empty State).
